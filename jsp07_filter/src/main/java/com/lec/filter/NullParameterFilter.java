...
/*
	...
	응답을 취소할 수 있다. 즉, Http의 request와 response의 내용을 변경할 수 있도록 지원하는 객체이다.
	
	필터는 객체형태로 클라이언트에서 요구하는 요청과 웹서버(jsp, servlet ...) 의 사이에 위치하여
	클라이언트의 요청을 제어할 수 있도록 한다. 또한, 웹서버와 응답처리 사이에 위치하여 응답을 제어할 수 있다.
	
	즉, 필터는 웹브라우저의 요청을 필터의 체인으로 연결해서 다음 단계로 보낼 수 있고, 
	다른 형태의 결과를 웹브라우저에 전송할 수 있다.
	
	A. 필터를 구현하기 위해서는 3개의 핵심 class가 필요하다.
	
	  1. javax.servlet.Filter : 웹브라우저와 최종 resource 사이에 위치하는 필터를 구현할 인터페이스
	  2. javax.servlet.ServletRequestMapper : 필터가 요청한 결과를 저장하는 클래스
	  2. javax.servlet.ServletResponseMapper : 필터가 응답한 결과를 저장하는 클래스
	
	B. Filter인터페이스의 관련 메서드
	
	  1. init() : 필터를 초기화할 때 사용하는 메서드
	  2. doFilter() : 필터 기능을 수행하는 추상메서드, FilterChain.chaint() 메서드를 이용해서 다음 필터로 이동
	...
	2. doFilter()
	  ...
	  a. request파라미터를 이용해서 요청의 필터 작업을 수행
...
public class NullParameterFilter implements Filter {
	...
	public void init(FilterConfig conf) throws ServletException {
		...
		paramNames = new String[st.countTokens()];  // 배열 생성 (크기)
