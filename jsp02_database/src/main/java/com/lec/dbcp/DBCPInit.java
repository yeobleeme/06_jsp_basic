...
public class DBCPInit extends HttpServlet {
	...
	private void initConnectionPool() {
		...
		// 1. 커넥션풀이 새로운 커넥션을 생성할 때 사용하는 커넥션팩토리 생성
		ConnectionFactory cf = new DriverManagerConnectionFactory(url, usr, pwd);
	
		// 2. PoolableConnection생성 - 팩토리생성 DBCP는 커넥션풀에 커넥션을 보관할 때
		//                             PoolableConnection을 사용한다. 이 클래스는 내부적으로 
		//                             커넥션을 보관하고 있으며 커넥션을 관리하는 데 필요한 기능을 제공한다.
		//                             예를 들어, connection을 close하면 connection을 메모리에서 
		//                             삭제하는 것이 아니라 Connection Pool에 Connection을 반환 한다.
		PoolableConnectionFactory pcf = new PoolableConnectionFactory(cf, null);	
		
		// 3. Connection이 유효한지 검사하기 위한 SQL 지정
		...
		// 1. PoolableConnection을 생성할 때 사용할 커넥션팩토리와 설정정보를 이용해 커넥션풀을 생성
		GenericObjectPool<PoolableConnection> cp = new GenericObjectPool<>(pcf, poolConfig);
		
		// 2. 생성된 커넥션을 풀에 연결
		pcf.setPool(cp);
		
		// D. Connection Pool에 제공할 JDBC드라이버 등록
		try {
