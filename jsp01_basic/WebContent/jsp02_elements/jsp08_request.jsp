...
<!--  
    ...
    A. request객체가 제공하는 기능
      ...
      2. 서버와 관련된 정보
      3. 클라이언트가 전송한 요청파라미터 정보(http://localhost:8088/login?id=hong&pw=12345)
      4. 클라이언트가 전송한 요청헤더 정보(http구역에 따른 정보 등..)
      5. 클라이언트가 전송한 쿠키 정보
      6. 속성처리 정보
    
    B. request객체의 요청파라미터 관련 메서드
    
      1. getParameter(String name) : name인 파리미터의 값을 리턴, 없으면 null
      2. getParameterValues(String name) : name인 파리미터의 값을 배열로 리턴, 없으면 null
      3. getParameterNames() : 웹브라우저가 전송한 파라미터의 이름목록을 배열로 리턴, 없으면 null
      4. getParameterMap() : 웹브라우저가 전송한 파라미터이름과 값을 Map타입으로 리턴, 없으면 null
    ...
    D. request객체의 메서드
      ...
      5. getProtocol() : 클라이언트가 요청한 protocol(http, ftp ..) 프로토콜 정보
...
<body>
	...
	<h3> 7. 요청정보의 URL: <%= request.getRequestURL() %></h3>
	<h3> 8. 요청정보의 컨텍스트경로: <%= request.getContextPath() %></h3>
