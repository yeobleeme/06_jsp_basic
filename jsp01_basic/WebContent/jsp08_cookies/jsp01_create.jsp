...
<!--  
	...
	K. 쿠키의 유효기간
	  ...
	  쿠키를 함께 삭제한다. 웹브라우저 종료 후 다시 웹브라우저를 실행하면 삭제한 쿠키는 
	  서버에 전송되지 않는다. 쿠키의 유효시간을 정해 놓으면 유효시간 동안 쿠키가 존재하고
	  웹브라우저가 종료되도 유효시간이 지나지 않았다면 그 쿠키는 삭제되지 않는다.
	  
	  유효시간을 지정하려면 setMaxAge()메서드를 사용한다. 유효시간 단위는 초단위로 지정한다.
	
	
	L. 쿠키와 헤더
	
	  response.addCookie()메서드로 쿠키를 추가하면 실제로 "Set-Cookie"헤더를 통해서 전달된다.
	  한 개의 Set-Cookie헤더는 한 개의 쿠기값을 전달한다. 헤더의 구성은 다음과 같다.
	  
	  Set-Cookie : 쿠키이름=쿠키값;Domain=도메인값;Path=경로;Expires=GMT형식의유효시간
-->
<!DOCTYPE html>
<html>
<head>
