...
<body>
	<!--  
		실제 도메인이 없기 때문에 localhost로는 테스트를 할 수 없다.
	...
	<%= cookie1.getName() %> = <%= cookie1.getValue() %> [<%= cookie1.getName() %>]
	<%= cookie2.getName() %> = <%= cookie2.getValue() %> [<%= cookie2.getName() %>]
	<%= cookie3.getName() %> = <%= cookie3.getValue() %> [<%= cookie3.getName() %>]
</body>
</html>